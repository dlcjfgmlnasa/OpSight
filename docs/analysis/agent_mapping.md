# Biosignal-FM Agents → OpSight 매핑 (Agent Mapping)
...
→ **결론**: data 처리 + 논문 + planning + 평가는 재사용 가치가 크다. 그러나 `model-architect`의 "PyTorch foundation model 빌더" 정체성은 OpSight에 그대로는 들어맞지 않는다.
...
| Agent | 근거 |
|-------|------|
| (없음) | BFM 5개 중 OpSight 환경에서 *전혀 수정 없이* 100% 들어맞는 것은 없다. paper-writer가 가장 가깝지만 venue / 톤 가이드를 npj DM에 맞춰 미세 조정해야 함 → (B)로 분류 |
...
| Agent | 폐기 근거 |
|-------|-----------|
| **model-architect** | OpSight는 PyTorch foundation model을 새로 학습하지 않는다. Transformer / MoE / RoPE / PatchEmbedding 지식은 OpSight의 LangGraph + tool-using workflow와 직접 연결되는 책임 영역이 없다. 향후 OpSight가 BFM checkpoint를 *호출*해서 embedding을 쓴다면 그 wrapper 역할은 `signal-ingest-engineer` 또는 신설 agent가 흡수하면 충분. **이름과 페르소나를 그대로 가져오면 잘못된 코드 (PyTorch 학습 loop)를 작성하려 할 위험이 있어 폐기를 권한다.** |

### (D) 새로 추가 권장 — 3개

| 새 Agent | 책임 | 신설 근거 |
|----------|------|-----------|
| **langgraph-engineer** | LangGraph StateGraph / Node / Edge 구현, tool registry, 이중 모드 분기 (Shallow 30초 ↔ Deep on-demand) 구현, state schema 설계 | OpSight의 *심장*. BFM에는 동치 agent 없음. `plan_langgraph.md` 기반 workflow로 운영. |
| **llm-prompt-engineer** | System prompt 작성, tool description 정제, role / instruction 분리, hallucination 억제 가드 작성, 한 / 영 대응 | LLM agent의 품질은 prompt에서 결정된다. PyTorch project에는 없던 책임 — OpSight에서 별도 agent로 두면 paper-writer / clinical-evaluator와의 분업이 깨끗해진다. |
...
- **Model: `opus`**: 동일 유지 (Opus 4.7이 OpSight 기준 시점의 최신).

## 5. 이식 시 정리할 작은 부채 (Small Debts to Clean Up)

- BFM의 일부 agent가 memory path를 `C:\Users\SNUH_VitalLab_LEGION\.claude\agent-memory\` (user-scope)로 적어두고 frontmatter는 `memory: project`로 선언 → 불일치. OpSight에선 모두 `C:\Projects\OpSight\.claude\agent-memory\<agent>\` (project-scope)로 통일한다.
