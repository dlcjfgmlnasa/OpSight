...
- Generalizable design — 비심장 주요 수술 (non-cardiac major surgery; general / thoracic / urologic / gynecologic) 에서 평가 (과대 "universal" claim 제거 — 2026-06-12 사장님 결정; 상세는 `docs/project_brief.md §1`)
...
- `opsight/tools/model_tools/` — 학습된 FM 백엔드 기반 tool. 플래그십 단일 `predict_hypotension` (저혈압 근미래 예측) — 현재 **Mock FM rule_based tier**(MAP 추세 외삽 + logistic; `mock_tier="rule_based"`), real FM(Stage 2)이 오면 `BiosignalFMInterface`로 교체. 나머지 후보는 `FM_DEFERRED_CANDIDATES`로 보류 (ADR-011/023). 백엔드 명칭은 그대로 FM (`category="fm"`).
