...
- `opsight/tools/model_tools/` — 학습된 FM 백엔드 기반 tool. 플래그십 단일 `predict_hypotension` (저혈압 조기 예측). 아직 deferred stub; 나머지 후보는 `FM_DEFERRED_CANDIDATES`로 보류 (ADR-023). 백엔드 명칭은 그대로 FM (`category="fm"` / `BiosignalFMInterface`).
